...
<template>
  <div class="settings">
    <base-nav :title="settingsData.title" class="settings__nav"></base-nav>
    <div class="settings__gap" ></div>
    <van-cell-group class="settings__cell" title="网络">
      <van-cell v-for="(netItem,index) in settingsData.networkData" :key="index" :title="netItem.title">
        <template #right-icon>
          <van-switch v-model="netItem.check" size="0.5rem" active-color="#c62f2f"/>
        </template>
      </van-cell>
    </van-cell-group>
    <div class="settings__gap" ></div>
    <van-cell-group class="settings__cell" title="播放和下载">
      <van-cell v-for="(playItem,index) in settingsData.playData" :key="index" :title="playItem.title" :value="playItem.value">
      </van-cell>
    </van-cell-group>
    <div class="settings__gap" ></div>
    <van-cell-group class="settings__cell" title="关于">
      <van-cell v-for="(aboutItem,index) in settingsData.aboutData" :key="index" :title="aboutItem.title" >
      </van-cell>
    </van-cell-group>
    <div class="settings__gap" ></div>
